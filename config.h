...
constexpr float SHIELD_ARC_ANGLE = 2.0f * PI / 3.0f; // Góc của khiên (120 độ)
constexpr float INITIAL_SHIELD_START_ANGLE = -PI / 10.3f; // Góc bắt đầu ban đầu của khiên
constexpr float SHIELD_ROTATION_SPEED_FACTOR = 2.0f * PI; // Tốc độ xoay khiên (rad/s)
constexpr float DEFAULT_SENSITIVITY = 50.0f; // Độ nhạy điều khiển khiên (0-100)
constexpr float MIN_SENSITIVITY_MULTIPLIER = 0.75f;
constexpr float MAX_SENSITIVITY_MULTIPLIER = 1.25f; // sensitivity_factor = 0.75 + (sensitivity/100)*0.5
const SDL_Color SHIELD_ARC_COLOR = {128, 0, 128, 255}; // Tím
const SDL_Color TRAJECTORY_CIRCLE_COLOR = {0, 0, 200, 255}; // Xanh dương đậm

// --- Kẻ địch: Tên lửa thường (Target) ---
constexpr float DEFAULT_MISSILE_SPEED = 100.0f; // Tốc độ cơ bản
constexpr float MAX_MISSILE_SPEED_RANDOM_FACTOR = 1.0f; // Tốc độ tối đa = default * (1 + rand)
constexpr int MISSILE_WIDTH = 45;
constexpr int MISSILE_HEIGHT = 30;
constexpr SDL_Point MISSILE_CENTER = {15, 30};
constexpr float MISSILE_COLLISION_RADIUS_SQ = 25.0f; // Bình phương bán kính va chạm với khiên

// --- Kẻ địch: Tên lửa nhanh (Fast Missile) ---
constexpr float FAST_MISSILE_SPEED_MULTIPLIER = 4.5f; // Tốc độ = missile_speed * multiplier
...
constexpr float WARNING_ALPHA_FREQ = 2.0f * PI * 2.0f / 1000.0f; // Tần số nhấp nháy alpha

// --- Kẻ địch: Space Shark ---
constexpr float SHARK_INITIAL_RADIUS = 300.0f;
constexpr float SHARK_MIN_RADIUS = 60.0f;
constexpr float SHARK_ANGULAR_SPEED = 2.0f; // rad/s
constexpr float SHARK_SPIRAL_SPEED = -20.0f; // Tốc độ đi vào tâm
constexpr Uint32 SHARK_LIFETIME = 15000; // Thời gian tồn tại (ms)
constexpr Uint32 SHARK_BULLET_INTERVAL = 5000; // Thời gian giữa các lần bắn đạn (ms)
constexpr int SHARK_WIDTH = 50;
constexpr int SHARK_HEIGHT = 30;
constexpr SDL_Point SHARK_CENTER = {25, 15};
constexpr float SHARK_COLLISION_RADIUS_SQ = 625.0f; // Va chạm với khiên (25*25)

// --- Kẻ địch: Shark Bullet ---
constexpr float SHARK_BULLET_SPEED_MULTIPLIER = 0.5f; // Tốc độ = default_missile_speed * multiplier
constexpr int SHARK_BULLET_WIDTH = 20;
constexpr int SHARK_BULLET_HEIGHT = 10;
constexpr SDL_Point SHARK_BULLET_CENTER = {10, 5};
constexpr float SHARK_BULLET_COLLISION_RADIUS_SQ = 100.0f; // Va chạm với khiên (10*10)

// --- Gameplay & Waves ---
constexpr int INITIAL_MISSILE_COUNT = 1;
constexpr int MAX_MISSILE_COUNT = 5;
constexpr int BASE_WAVES_UNTIL_INCREASE = 7; // Số wave cơ bản trước khi tăng số lượng tên lửa
constexpr int RANDOM_WAVES_UNTIL_INCREASE = 6; // Cộng thêm ngẫu nhiên vào số wave (0-5)
constexpr Uint32 INITIAL_SPAWN_DELAY = 2000; // Thời gian chờ ban đầu (ms)
constexpr Uint32 BASE_WAVE_DELAY = 3000; // Thời gian chờ giữa các wave (ms)
constexpr Uint32 RANDOM_WAVE_DELAY = 2001; // Cộng thêm ngẫu nhiên (0-2000 ms)
constexpr Uint32 MISSILE_SPAWN_INTERVAL = 300; // Thời gian tối thiểu giữa các tên lửa trong wave (ms)
...
constexpr int WAVE_INTERVAL_FAST_MISSILE = 3; // Xuất hiện mỗi 3 wave sau đó
constexpr int WAVE_START_SHARK = 15; // Wave bắt đầu xuất hiện cá mập
constexpr int WAVE_INTERVAL_SHARK = 15; // Xuất hiện mỗi 15 wave sau đó
...
const SDL_Rect VOLUME_SLIDER_RECT_SETTINGS = { (SCREEN_WIDTH - BUTTON_WIDTH) / 2, 280, BUTTON_WIDTH, 10 }; // Điều chỉnh Y
const SDL_Rect VOLUME_KNOB_RECT_SETTINGS = { VOLUME_SLIDER_RECT_SETTINGS.x + (DEFAULT_VOLUME * VOLUME_SLIDER_RECT_SETTINGS.w / 100) - 5, 275, 10, 20 }; // Điều chỉnh Y
// *** FIX: Changed constexpr to const ***
const int VOLUME_LABEL_Y_SETTINGS = VOLUME_SLIDER_RECT_SETTINGS.y - 40;

const SDL_Rect SENSITIVITY_SLIDER_RECT_SETTINGS = { (SCREEN_WIDTH - BUTTON_WIDTH) / 2, 380, BUTTON_WIDTH, 10 }; // Điều chỉnh Y
const SDL_Rect SENSITIVITY_KNOB_RECT_SETTINGS = { SENSITIVITY_SLIDER_RECT_SETTINGS.x + (int)(DEFAULT_SENSITIVITY * SENSITIVITY_SLIDER_RECT_SETTINGS.w / 100.0f) - 5, 375, 10, 20 }; // Điều chỉnh Y
// *** FIX: Changed constexpr to const ***
...
constexpr int ARC_SEGMENTS = 24; // Số đoạn vẽ cung tròn
